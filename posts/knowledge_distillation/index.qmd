...
The probability of the i-th token logit $x_i$ is given by:
...
1. Forward KL divergence: $$\sum p(x) log[\frac{p(x)}{q(x)}]$$

2. Reverse KL divergence: $$\sum q(x) log[\frac{q(x)}{p(x)}]$$

3. JS divergence: $$\frac{1}{2}[\sum p(x) log[\frac{2p(x)}{p(x)+q(x)}] + \sum q(x) log[\frac{2q(x)}{p(x)+q(x)}]]$$
...
$$L_{is} = \frac{1}{l} \sum_{k \in H} \sum_{i=1}^{l} loss(h_{t, i}^{(k)}, h_{s, i}^{(k)}) $$

where

* $(h_{t, i}^{(k)}, h_{s, i}^{(k)})$ represents the kth hidden state for the i-th token. 
...
The hidden states used are often after the LayerNorm operation.
...
* $XW^{Q, i}, XW^{K, i}, XW^{V, i} \in \mathbb{R}^{d_{model} \times d_{head}}$
