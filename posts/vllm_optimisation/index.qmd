...
2. **Decode**: The model processes the prompt and generates the next token -  "the". To generate this, the model performs an attention calculation. Paged Attention looks at block 7, fetches the KV cache & computes attention score.

To store the KV vectors for token "the", we enter into a new row in the lookup table. Let this be logical block 1 and physical block 12. The vectors are saved here
